...
for i, data in enumerate(validation_loader):
    ...
    for j in range(2):
        if j == 0:
            ...
            rgb, weights = render_rgb_depth(model[j], ray_flat, t_vals, ray_views, NUM_SAMPLES)
            ...
            rgb, weights = render_rgb_depth(model[j], ray_flat, t_vals, ray_views, NUM_FINE + NUM_SAMPLES)
    ...
    for j in range(BATCH_SIZE):
        ...
        if (i * BATCH_SIZE + j) % (DIMENSIONS * DIMENSIONS) == (DIMENSIONS * DIMENSIONS) - 1:
            ...
            plt.savefig('./results/dual/validation/' + str(((i * BATCH_SIZE + j) // (DIMENSIONS * DIMENSIONS)) + 1) + '.jpg')
            if VIDEO:
